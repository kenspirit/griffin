defmodule Lolth.Spider.Impl.NasdaqStockBasic do
  use Lolth.Spider.Impl
  use Tesla

  @domain "https://api.nasdaq.com/api/screener/stocks"
  @limit 100
  ...
  defp get_by_page(offset) do
    result = get(@domain,
      query: [tableonly: true, limit: @limit, offset: offset, exchange: "NASDAQ"],
    ...
    case result do
      {:ok, %{status: 200} = response} ->
        ...
        case status["rCode"] do
          200 ->
            save_gem_in_batch(data["table"]["rows"], offset)
          _ ->
